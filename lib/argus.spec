...
Requires: wget
Requires: cyrus-sasl
Conflicts: argus-systemd-workstation < 5.0-3.2%{dist}.2
Conflicts: argus-systemd-server < 5.0-3.2%{dist}.2

%description
Argus-5.0 is the latest release of Argus (Audit Record Generation And Utilization System),
which is a data network transaction auditing tool.  The data generated by argus can be used
...
%build
%configure --with-sasl %{?_flexlm:--with-flexlm=%{_flexlm}}
...
install -D -m 0700 pkg/rhel/systemd/argus-setup-numa $RPM_BUILD_ROOT/%{argussbin}/argus-setup-numa
...
%{argusbin}/argus-ipaddr
%{argusbin}/argus-top
%{argusbin}/argus-ipmitool
%{argusbin}/argusbug
...
Conflicts: argus < 5.0-3.0%{dist}.2
BuildArch: noarch
...
%attr(644,root,root) %{_unitdir}/argus@.service
%{argussbin}/argus-setup
%{argussbin}/argus-setup-numa
